...
void fletcher_reeves(tInfo &info) {

    tvecDouble grad(2), dk(2), newx(2);
    double norma_grad = 0, norma_grad_pred = 0, l;
    int k = 0;
    int f = -1;
    while (true) {
        ...
        if (norma_grad > info.delta) {
            ...
            if (k == 0) {
                ...
                dk[0] = -grad[0] + (pow(norma_grad, 2) / pow(norma_grad_pred, 2)) * dk[0];
                dk[1] = -grad[1] + (pow(norma_grad, 2) / pow(norma_grad_pred, 2)) * dk[1];
            }

            tInfo _info(-1, 1, 0, 1E-5);

            l = dichotomy(_info, info, dk);

            newx[0] = info.x0 + l * dk[0];
            newx[1] = info.x1 + l * dk[1];

            if ((sqrt(pow(newx[0] - info.x0, 2) + pow(newx[1] - info.x1, 2)) < info.delta) &&
